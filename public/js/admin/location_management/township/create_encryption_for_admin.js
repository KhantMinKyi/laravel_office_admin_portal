...
$(document).on("click", "#btnDecryptCreateTownshipForAdmin", function () {
    var selectedOption = $("#create_township_for_admin_encryption_key").find(
        "option:selected"
    );
    var encryptedKey = selectedOption.data(
        "create_township_for_admin_encryption_key"
    );
    $(".data_input_create_township_for_admin").each(function (index) {
        ...
        $(".data_input_create_township_for_admin")
            .eq(index)
            .text(pkcs7Unpad(result));
    });
});

$(document).on("click", "#btnEncryptCreateTownshipForAdmin", function () {
    var selectedOption = $("#create_township_for_admin_encryption_key").find(
        "option:selected"
    );
    var encryptedKey = selectedOption.data(
    ...
    $(".data_input_create_township_for_admin").each(function (index) {
        ...
        $(".data_input_create_township_for_admin").eq(index).text(btoa(result));
    });
});
