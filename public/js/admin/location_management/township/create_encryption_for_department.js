...
$(document).on("click", "#btnDecryptCreateTownshipForDepartment", function () {
    var selectedOption = $(
        "#create_township_for_department_encryption_key"
    ).find("option:selected");
    var encryptedKey = selectedOption.data(
        "create_township_for_department_encryption_key"
    );
    $(".data_input_create_township_for_department").each(function (index) {
        ...
        $(".data_input_create_township_for_department")
            .eq(index)
            .text(pkcs7Unpad(result));
    });
});

$(document).on("click", "#btnEncryptCreateTownshipForDepartment", function () {
    var selectedOption = $(
        "#create_township_for_department_encryption_key"
    ).find("option:selected");
    var encryptedKey = selectedOption.data(
