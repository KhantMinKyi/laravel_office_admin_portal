...
$(document).on("click", "#btnDecryptEditTownshipForAdmin", function () {
    var selectedOption = $("#edit_township_for_admin_encryption_key").find(
        "option:selected"
    );
    var encryptedKey = selectedOption.data(
        "edit_township_for_admin_encryption_key"
    );
    $(".data_input_edit_township_for_admin").each(function (index) {
        ...
        $(".data_input_edit_township_for_admin")
            .eq(index)
            .text(pkcs7Unpad(result));
    });
});

$(document).on("click", "#btnEncryptEditTownshipForAdmin", function () {
    var selectedOption = $("#edit_township_for_admin_encryption_key").find(
        "option:selected"
    );
    var encryptedKey = selectedOption.data(
    ...
    $(".data_input_edit_township_for_admin").each(function (index) {
        ...
        $(".data_input_edit_township_for_admin").eq(index).text(btoa(result));
    });
});
