...
$(document).on("click", "#btnDecryptViewTownshipForBranch", function () {
    var selectedOption = $("#view_township_for_branch_encryption_key").find(
        "option:selected"
    );
    var encryptedKey = selectedOption.data(
        "view_township_for_branch_encryption_key"
    );
    $(".data_input_view_township_for_branch").each(function (index) {
        ...
        $(".data_input_view_township_for_branch")
            .eq(index)
            .text(pkcs7Unpad(result));
    });
});

$(document).on("click", "#btnEncryptViewTownshipForBranch", function () {
    // alert("asdasd");
    var selectedOption = $("#view_township_for_branch_encryption_key").find(
        "option:selected"
    );
    ...
    $(".data_input_view_township_for_branch").each(function (index) {
        ...
        $(".data_input_view_township_for_branch").eq(index).text(btoa(result));
    });
});
