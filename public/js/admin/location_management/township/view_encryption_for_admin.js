...
$(document).on("click", "#btnDecryptViewTownshipForAdmin", function () {
    var selectedOption = $("#view_township_for_admin_encryption_key").find(
        "option:selected"
    );
    var encryptedKey = selectedOption.data(
        "view_township_for_admin_encryption_key"
    );
    $(".data_input_view_township_for_admin").each(function (index) {
        ...
        $(".data_input_view_township_for_admin")
            .eq(index)
            .text(pkcs7Unpad(result));
    });
});

$(document).on("click", "#btnEncryptViewTownshipForAdmin", function () {
    // alert("asdasd");
    var selectedOption = $("#view_township_for_admin_encryption_key").find(
        "option:selected"
    );
    ...
    $(".data_input_view_township_for_admin").each(function (index) {
        ...
        $(".data_input_view_township_for_admin").eq(index).text(btoa(result));
    });
});
