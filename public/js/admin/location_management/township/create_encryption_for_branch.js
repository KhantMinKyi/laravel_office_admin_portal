...
$(document).on("click", "#btnDecryptCreateTownshipForBranch", function () {
    var selectedOption = $("#create_township_for_branch_encryption_key").find(
        "option:selected"
    );
    var encryptedKey = selectedOption.data(
        "create_township_for_branch_encryption_key"
    );
    $(".data_input_create_township_for_branch").each(function (index) {
        ...
        $(".data_input_create_township_for_branch")
            .eq(index)
            .text(pkcs7Unpad(result));
    });
});

$(document).on("click", "#btnEncryptCreateTownshipForBranch", function () {
    var selectedOption = $("#create_township_for_branch_encryption_key").find(
        "option:selected"
    );
    var encryptedKey = selectedOption.data(
