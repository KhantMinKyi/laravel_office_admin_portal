...
$(document).on("click", "#btnDecryptEditCityForTownship", function () {
    var selectedOption = $("#edit_city_for_township_encryption_key").find(
        "option:selected"
    );
    var encryptedKey = selectedOption.data(
        "edit_city_for_township_encryption_key"
    );
    $(".data_input_edit_city_for_township").each(function (index) {
        ...
        $(".data_input_edit_city_for_township")
            .eq(index)
            .text(pkcs7Unpad(result));
    });
});

$(document).on("click", "#btnEncryptEditCityForTownship", function () {
    var selectedOption = $("#edit_city_for_township_encryption_key").find(
        "option:selected"
    );
    var encryptedKey = selectedOption.data(
    ...
    $(".data_input_edit_city_for_township").each(function (index) {
        ...
        $(".data_input_edit_city_for_township").eq(index).text(btoa(result));
    });
});
