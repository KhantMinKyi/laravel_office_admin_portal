...
$(document).on("click", "#btnDecryptCreateCityForDepartment", function () {
    var selectedOption = $("#create_city_for_department_encryption_key").find(
        "option:selected"
    );
    var encryptedKey = selectedOption.data(
        "create_city_for_department_encryption_key"
    );
    $(".data_input_create_city_for_department").each(function (index) {
        ...
        $(".data_input_create_city_for_department")
            .eq(index)
            .text(pkcs7Unpad(result));
    });
});

$(document).on("click", "#btnEncryptCreateCityForDepartment", function () {
    var selectedOption = $("#create_city_for_department_encryption_key").find(
        "option:selected"
    );
    var encryptedKey = selectedOption.data(
