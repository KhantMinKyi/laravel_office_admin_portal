...
$(document).on("click", "#btnDecryptViewCityForTownship", function () {
    var selectedOption = $("#view_city_for_township_encryption_key").find(
        "option:selected"
    );
    var encryptedKey = selectedOption.data(
        "view_city_for_township_encryption_key"
    );
    $(".data_input_view_city_for_township").each(function (index) {
        ...
        $(".data_input_view_city_for_township")
            .eq(index)
            .text(pkcs7Unpad(result));
    });
});

$(document).on("click", "#btnEncryptViewCityForTownship", function () {
    var selectedOption = $("#view_city_for_township_encryption_key").find(
        "option:selected"
    );
    var encryptedKey = selectedOption.data(
    ...
    $(".data_input_view_city_for_township").each(function (index) {
        ...
        $(".data_input_view_city_for_township").eq(index).text(btoa(result));
    });
});
