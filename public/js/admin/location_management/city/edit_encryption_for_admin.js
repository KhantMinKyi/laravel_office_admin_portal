...
$(document).on("click", "#btnDecryptEditCityForAdmin", function () {
    var selectedOption = $("#edit_city_for_admin_encryption_key").find(
        "option:selected"
    );
    var encryptedKey = selectedOption.data(
        "edit_city_for_admin_encryption_key"
    );
    $(".data_input_edit_city_for_admin").each(function (index) {
        ...
        $(".data_input_edit_city_for_admin").eq(index).text(pkcs7Unpad(result));
    });
});

$(document).on("click", "#btnEncryptEditCityForAdmin", function () {
    var selectedOption = $("#edit_city_for_admin_encryption_key").find(
        "option:selected"
    );
    var encryptedKey = selectedOption.data(
    ...
    $(".data_input_edit_city_for_admin").each(function (index) {
        ...
        $(".data_input_edit_city_for_admin").eq(index).text(btoa(result));
    });
});
