...
$(document).on("click", "#btnDecryptEditCityForBranch", function () {
    var selectedOption = $("#edit_city_for_branch_encryption_key").find(
        "option:selected"
    );
    var encryptedKey = selectedOption.data(
        "edit_city_for_branch_encryption_key"
    );
    $(".data_input_edit_city_for_branch").each(function (index) {
        ...
        $(".data_input_edit_city_for_branch")
            .eq(index)
            .text(pkcs7Unpad(result));
    });
});

$(document).on("click", "#btnEncryptEditCityForBranch", function () {
    var selectedOption = $("#edit_city_for_branch_encryption_key").find(
        "option:selected"
    );
    var encryptedKey = selectedOption.data(
    ...
    $(".data_input_edit_city_for_branch").each(function (index) {
        ...
        $(".data_input_edit_city_for_branch").eq(index).text(btoa(result));
    });
});
