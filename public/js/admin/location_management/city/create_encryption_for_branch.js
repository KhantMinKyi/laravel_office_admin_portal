...
$(document).on("click", "#btnDecryptCreateCityForBranch", function () {
    var selectedOption = $("#create_city_for_branch_encryption_key").find(
        "option:selected"
    );
    var encryptedKey = selectedOption.data(
        "create_city_for_branch_encryption_key"
    );
    $(".data_input_create_city_for_branch").each(function (index) {
        ...
        $(".data_input_create_city_for_branch")
            .eq(index)
            .text(pkcs7Unpad(result));
    });
});

$(document).on("click", "#btnEncryptCreateCityForBranch", function () {
    var selectedOption = $("#create_city_for_branch_encryption_key").find(
        "option:selected"
    );
    var encryptedKey = selectedOption.data(
    ...
    $(".data_input_create_city_for_branch").each(function (index) {
        ...
        $(".data_input_create_city_for_branch").eq(index).text(btoa(result));
    });
});
