...
$(document).on("click", "#btnDecryptEditCityForDepartment", function () {
    var selectedOption = $("#edit_city_for_department_encryption_key").find(
        "option:selected"
    );
    var encryptedKey = selectedOption.data(
        "edit_city_for_department_encryption_key"
    );
    $(".data_input_edit_city_for_department").each(function (index) {
        ...
        $(".data_input_edit_city_for_department")
            .eq(index)
            .text(pkcs7Unpad(result));
    });
});

$(document).on("click", "#btnEncryptEditCityForDepartment", function () {
    var selectedOption = $("#edit_city_for_department_encryption_key").find(
        "option:selected"
    );
    var encryptedKey = selectedOption.data(
    ...
    $(".data_input_edit_city_for_department").each(function (index) {
        ...
        $(".data_input_edit_city_for_department").eq(index).text(btoa(result));
    });
});
