    ...
    <div class="p-10 bg-gray-50 text-black dark:bg-mainbody-800 dark:text-white relative rounded">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                    @foreach ($branch->users as $key => $branch_user)
                        <tr
                            ...
                            <th scope="row"
                                ...
                                {{ $key + 1 }}</th>
                            <td class="px-4 py-5">{{ $branch_user->full_name }}</td>
                            <td class="px-4 py-5">{{ $branch_user->position }}</td>
                            <td class="px-4 py-5">{{ $branch->name }}</td>
                            <td class="px-4 py-5">{{ $branch_user->department->name }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
