...
<div class="flex items-center justify-center">
    <div class="p-10 bg-gray-50 text-black dark:bg-mainbody-800 dark:text-white relative rounded">
        <button class="absolute top-0 right-0 px-4 pt-4 " onclick="togglePopupDepartmentViewModel()">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                    @foreach ($department->users as $key => $department_user)
                        <tr
                            ...
                            <th scope="row"
                                ...
                                {{ $key + 1 }}</th>
                            <td class="px-4 py-5">{{ $department_user->full_name }}</td>
                            <td class="px-4 py-5">{{ $department_user->position }}</td>
                            <td class="px-4 py-5">{{ $department->branch->name }}</td>
                            <td class="px-4 py-5">{{ $department->name }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
