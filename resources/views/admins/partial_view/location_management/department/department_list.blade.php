...
<div class="bg-white dark:bg-mainbody-800 relative shadow-md sm:rounded-lg overflow-hidden">
    ...
    <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
        {{-- For City --}}
        <div>
            <select id="view_city_for_department_encryption_key" name="view_city_for_department_encryption_key"
                ...
                @foreach ($encryption_keys as $encryption_key)
                    <option value="{{ $encryption_key->key }}"
                        data-view_city_for_department_encryption_key="{{ $encryption_key->key }}">
        ...
        <div class="flex items-center space-x-3 w-full md:w-auto">
            <button id="actionsDropdownButtonDepartmentCity" data-dropdown-toggle="actionsDropdownDepartmentCity"
            ...
            <div id="actionsDropdownDepartmentCity"
                ...
                <ul class="py-1 text-sm text-gray-700 dark:text-gray-200"
                    aria-labelledby="actionsDropdownButtonDepartmentCity">
                    <li>
        ...
        {{-- For Township --}}
        <div>
            <select id="view_township_for_department_encryption_key" name="view_township_for_department_encryption_key"
                class="encryption_key bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-mainbody-300 focus:border-mainbody-300 block w-full p-2.5 dark:bg-mainbody-700 dark:border-mainbody-600 dark:placeholder-mainbody-300 dark:text-white dark:focus:ring-mainbody-800 dark:focus:border-mainbody-800">
                <option selected disabled>Select Township Encryption Key</option>
                @foreach ($encryption_keys as $encryption_key)
                    <option value="{{ $encryption_key->key }}"
                        data-view_township_for_department_encryption_key="{{ $encryption_key->key }}">
                        {{ $encryption_key->key_description->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="flex items-center space-x-3 w-full md:w-auto">
            <button id="actionsDropdownButtonTownshipForDepartment"
                data-dropdown-toggle="actionsDropdownTownshipForDepartment"
                class="w-full md:w-auto flex items-center justify-center py-2 px-4 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-mainbody-700 focus:z-10 focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 dark:bg-mainbody-800 dark:text-gray-400 dark:border-mainbody-600 dark:hover:text-white dark:hover:bg-mainbody-700"
                type="button">
                <svg class="-ml-1 mr-1.5 w-5 h-5" fill="currentColor" viewbox="0 0 20 20"
                    xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
            ...
            <div id="actionsDropdownTownshipForDepartment"
                class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-mainbody-700 dark:divide-mainbody-600">
                <ul class="py-1 text-sm text-gray-700 dark:text-gray-200"
                    aria-labelledby="actionsDropdownButtonTownshipForDepartment">
                    <li>
        ...
        <div
            ...
            <button type="button" id="add-button" onclick="togglePopupDepartmentAddModel()"
            ...
            <tbody>
                @foreach ($departments as $depaetment_key => $department)
                    <tr
                        ...
                        <td class="px-4 py-3 data_input_view_city_for_department ">{{ $department->city->name }}</td>
                        <td class="px-4 py-3 data_input_view_township_for_department">{{ $department->township->name }}
                        </td>
                        ...
                        <td class="px-4 py-3 flex items-center justify-end">
                            ...
                            <div id="department-imac-{{ $department->id }}-dropdown"
                                class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-mainbody-700 dark:divide-mainbody-600">
                                <ul class="py-1 text-sm text-gray-700 dark:text-gray-200"
                                    ...
                                    <li>
                                        <button
                                            ...
                                            onclick="togglePopupDepartmentEditModel({{ $department->id }})">Edit</button>
                                    </li>
                                </ul>
                                <div class="py-1">
                                    <a href="#"
...
<script src="/js/admin/location_management/township/view_encryption_for_department.js"></script>
<script src="/js/admin/location_management/city/view_encryption_for_department.js"></script>
