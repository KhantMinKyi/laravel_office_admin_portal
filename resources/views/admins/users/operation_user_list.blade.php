...
@section('content')
    ...
    ?>
    {{-- Operation User List table --}}
    <div class="p-4  relative " wire:key='operation-user-list'>
        <section class=" dark:bg-mainbody-900 p-3 sm:p-5">
            <div class="mx-auto  px-4 lg:px-12">
                <div class="my-2 w-1/3">
                    <h3 class="text-xl font-semibold text-mainbody-300 dark:text-gray-100 ">Operation List</h3>
                ...
                <div class="bg-white dark:bg-mainbody-800 relative shadow-md sm:rounded-lg overflow-hidden">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400 results">
                            ...
                            <tbody>
                                @foreach ($operation_users as $key => $operation_user)
                                    <tr
                                        ...
                                        <th scope="row"
                                            ...
                                            {{ $operation_user->full_name }}</th>
                                        <td class="px-4 py-3">{{ $operation_user->username }}</td>
                                        <td class="px-4 py-3">{{ $operation_user->email }}</td>
                                        <td class="px-4 py-3">{{ $operation_user->department->name }}</td>
                                        <td class="px-4 py-3">{{ $operation_user->position }}</td>
                                        <td class="px-4 py-3 flex items-center justify-end"
                                            data-id="{{ $operation_user->id }}">
                                            {{-- view button --}}
                                            ...
                                            {{-- view dot --}}
                                            <button id="apple-imac-{{ $operation_user->id }}-dropdown-button"
                                                data-dropdown-toggle="apple-imac-{{ $operation_user->id }}-dropdown"
                                            ...
                                            <div id="apple-imac-{{ $operation_user->id }}-dropdown"
                                                class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-mainbody-700 dark:divide-mainbody-600">
                                                <ul class="py-1 text-sm text-gray-700 dark:text-gray-200"
                                                ...
                                                <a href="#"
                                                    ...
                                                    <li>
                                                        <button
                                                            ...
                                                            onclick="togglePopupEditModel({{ $operation_user->id }})">Edit</button>
                                                    </li>
                                                </ul>
                                                <div class="py-1">
                                                    <a href="#"
        ...
        <div class="hidden fixed top-32 right-5 bottom-0 xl:left-72 md:left-1/4 overflow-y-auto" id="popup-model">
            ...
            <div class="flex items-center justify-center">
                <div class="p-10 bg-white text-black dark:bg-mainbody-800 dark:text-white relative rounded">
                    ...
                    <div>
                        <h4 class="flex justify-center mb-4 font-semibold text-gray-500 dark:text-gray-100">Add Operation
                            Form
                        </h4>
                        {{-- Include Create User --}}
                        @include('admins.partial_view.users.operation_user.create_user')
                    </div>
        ...
        <div class="hidden fixed top-32 right-5 bottom-0 xl:left-72 md:left-1/4 overflow-y-auto" id="popup-edit-model">
            {{-- Include Edit User --}}
            {{-- @include('admins.partial_view.users.operation_user.edit_user') --}}
        </div>
